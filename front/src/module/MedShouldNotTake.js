...
const MedShouldNotTake = ({ username, medicineName }) => {
  const [medicationInfo, setMedicationInfo] = useState(['나', '다', '가', '나']); // 더미 데이터 -> 추후 삭제
  ...
  const handleDeleteMedication = async () => { //약 삭제 함수
    if (selectedMedIndex !== null) {
      // 로컬에서 삭제
      const updatedMedicationInfo = [...medicationInfo];
      const deletedMedication = updatedMedicationInfo.splice(selectedMedIndex, 1);
      ...
      // 약 삭제 요청
      try {
        await axios.delete(`http://localhost:8000/deleteMedication/${username}`, {
  ...
  return (
    <>
      <div className="deleteMed" onClick={handleDeleteMedication}>
        <img src={deleteCircle} alt="Delete Circle" />
        약 정보 삭제하기
      </div>
      <div className="MedShouldNotTakeContainer">
        ...
        <div className="Med">
          {medicationInfo.map((med, index) => (
            <div
              className={`addMedCircle ${selectedMedIndex === index ? "selected" : ""}`}
              key={index}
